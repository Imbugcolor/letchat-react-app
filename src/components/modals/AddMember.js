...
import { addMember } from "../../redux/actions/message.action";

const AddMember = () => {
  const auth = useSelector(state => state.auth);
  const conversation = useSelector(state => state.modal).addMember;
  ...
  const handleAddMember = async(e) => {
    e.preventDefault()
    setLoad(true)
    await dispatch(addMember({ auth, users: usersSelected, conversationId: conversation.id }))
    setLoad(false)
  };
  ...
  const handleSearch = async () => {
    setFirstSearch(true);
    const usersMatch = await getDataAPI(
      `users/search?searchTerm=${searchInput}`,
      auth.token,
      dispatch
    );
          ...
          >
            ...
            <div
              className={`search-results ${
                users && users.length > 4 && "overflow-y-scroll"
              }`}
            >
              {users &&
                users.length > 0 &&
                users.map((user) => (
                  ...
                  >
                    ...
                    {
                        conversation.participants.every(part => part.user.id !== user.id) &&
                        <div
